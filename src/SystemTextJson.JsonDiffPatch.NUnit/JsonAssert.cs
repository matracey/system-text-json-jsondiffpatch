...
namespace System.Text.Json.JsonDiffPatch.Nunit
{
    ...
    public static class JsonAssert
    {
        ...
        private static void HandleAreEqual(JsonNode? expected, JsonNode? actual,
            ...
            Func<JsonNode, string>? outputFormatter)
        {
            var constraint = JsonIs.EqualTo(expected);

            if (diffOptions is not null)
            {
                constraint = constraint.WithDiffOptions(diffOptions);
            }

            if (outputFormatter is not null)
            {
                constraint = constraint.WithOutputFormatter(outputFormatter);
            }

            Assert.That(actual, constraint, "JsonAssert.AreEqual() failure.");
        }
        ...
        {
            var constraint = JsonIs.NotEqualTo(expected);
            ...
            Assert.That(actual, constraint, "JsonAssert.AreNotEqual() failure.");
        }
    }
}